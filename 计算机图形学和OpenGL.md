...
1. 计算机图形学是从模型到图片，计算机视觉是从图片到模型。

2. ![image-20210521162829103](计算机图形学和OpenGL.assets/image-20210521162829103.png)

3. 图形学中的向量一般都用单位向量$\hat{a}$。$b_{\perp}$是向量$\vec{b}$在向量$\vec{a}$上的投影，$=(b\cdot\hat{a})\hat{a}$。
...
5. 也可以写成矩阵表示$A^*$是$\vec{a}$​的对偶矩阵：
   $$
   \vec{a}\times\vec{b}=A^*b=\begin{pmatrix}0&-z_a&y_a\\z_a&0&-x_a\\-y_a&x_a&0\end{pmatrix}\begin{pmatrix}x_b\\y_b\\z_b\end{pmatrix}
   $$

6. 叉乘可以用来判断平面向量的左右，例如$\vec{a}\times\vec{b}$的方向是$+Z$方向，则说明$\vec{b}$在$\vec{a}$的左侧：

7. ![image-20210521202431410](计算机图形学和OpenGL.assets/image-20210521202431410.png)

8. 叉积还可以用来判定平面上点P是否在三角形ABC内。分别用$\vec{AB}\times\vec{AP}$，$\vec{BC}\times\vec{BP}$，$\vec{CA}\times\vec{CP}$。三个结果向量如果是同向的，则P点在三角形内部。

11. ![image-20210521202620105](计算机图形学和OpenGL.assets/image-20210521202620105.png)

# 变换

1. 旋转默认是以原点为中心，逆时针转动。
2. 平移变换是非齐次的，不属于线性变换，因为不能表示为一个矩阵左乘原来的位置坐标。常见的线性变换有：缩放，旋转（2维绕点，3维绕轴）。

## 齐次坐标

1. 引入齐次坐标的目的是为了将平移变换和其他的线性变换以统一的形式表示。

2. 在平面2维点和向量的最后加上一维。向量具有平移不变性，而点不具有。这样的规定也是符合向量和点之间的关系的，即向量=点-点。
...
3. 向量+向量结果是向量，向量+点结果是点，点-点结果是向量。点+点本来是无意义的，而在此被赋予新的含义：(x,y,w)如果w≠0，则(x,y,w)表示的点和(x/w,y/w,1)是相同的。几何意义就是两个点的中点。

4. 仿射变换=线性变换+平移变换。所有的仿射变换都可以写成齐次坐标的形式。先应用线性变换，再平移变换。
   $$
...
6. 这样平移变换就可以写成线性变换了。可以发现，下式对于向量，结果仍然是该向量。
   $$
   \begin{pmatrix}x'\\y'\\w'\end{pmatrix}=\begin{pmatrix}1&0&t_x\\0&1&t_y\\0&0&1\end{pmatrix}\cdot\begin{pmatrix}x\\y\\1\end{pmatrix}=\begin{pmatrix}x+t_x\\y+t_y\\1\end{pmatrix}
   $$
   
7. 变换就是做矩阵乘法，由于矩阵乘法没有交换律，因此变换的顺序不能随意交换。

8. 如果要绕任意一个点旋转，可以先将图形平移，使得该点位于原点，然后旋转，最后再反向平移，使得该点回到原来的位置。

9. ![image-20210521221837640](计算机图形学和OpenGL.assets/image-20210521221837640.png)
...
11. 旋转变换是正交变换，旋转$\theta$角的逆变换等于旋转$-\theta$。

12. 空间点的绕特定轴的旋转，可以分解为绕三个坐标轴的旋转的组合。$\alpha,\beta,\gamma$​被称为欧拉角。$R_{xyz}(\alpha,\beta,\gamma)=R_x(\alpha)R_y(\beta)R_z(\gamma)$

13. 罗德里格斯旋转公式，绕轴$\vec{n}$旋转$\alpha$，默认轴$\vec{n}$是过原点的，如果不是的，可以通过平移：

14. ![image-20210521223440917](计算机图形学和OpenGL.assets/image-20210521223440917.png)
...
3. 相机的摆放需要以下几个变量：
   ...
   2. 看的方向，look-at  $\vec{g}$
   3. 上方向，up direction  $\vec{t}$
4. 如果相机和模型一起移动，那么view视图是不变的，因此可以固定相机不动，让模型绕着相机运动。位置在原点，看着-Z，上方向是Y。正好类似于人眼观察屏幕。
5. 依次进行如下操作，将任意位置的相机摆放到标准位置：
   1. 将$\vec{e}$移动到原点
   2. 将$\vec{g}$旋转到-Z方向
   3. 将$\vec{t}$旋转到Y方向
   4. 将$\vec{g}\times\vec{t}$旋转到X方向 ，做完2,3步，这一步自然完成。
6. 将任意轴旋转到标准轴比较麻烦，可以先求标准轴到任意轴的旋转，然后求逆（转置）矩阵即可。
7. 下面的左侧矩阵会将标准正交基向量旋转到对应的$\vec{g}$和$\vec{t}$还有$\vec{g}\times\vec{t}$。
8. ![image-20210521230620683](计算机图形学和OpenGL.assets/image-20210521230620683.png)
9. 将模型和相机应用相同的方法，得到的view视图就是不变的。模型变换和视图变换都是应用在模型上，通常被称为模型视图变换。

## 投影变换

1. 常见的两种投影变换：正交投影，透视投影。
...
5. 人眼的成像是透视投影，应用最广泛，正交投影多用于工程制图。

6. 二者的区别是，透视投影会表现出近大远小的效果。

7. 透视投影：相机看做一个点，相机和投影面构成一个四棱锥，将在一定距离范围内，从near clip plane到far clip plane之间模型投影到near clip plane。投影面上过中心点法线是通过原点的。

8. ![image-20210521232221936](计算机图形学和OpenGL.assets/image-20210521232221936.png)

9. 透视投影的相机如果在无限远处，就变成了正交投影。

10. 获取标准视图下的正交投影，：

    1. 扔掉Z坐标，投影
    2. 将投影内容平移缩放到$[-1,1]^2$范围内。这一步会改变模型的长宽比。

11. 实际工程中是使用如下方法：

    1. 获取模型的三个方向的边界，从左到右，从下到上，从远到近。，将模型平移（中心移动到原点）缩放到$[-1,1]^3$。同时也要讲投影面进行相同的变换。
    2. 扔掉Z坐标，投影。

12. 需要注意的是，由于相机的观察是沿着-Z方向的，因此远处的坐标是<近处的坐标的。这是由于使用了右手坐标系的原因。

13. ![image-20210521233216802](计算机图形学和OpenGL.assets/image-20210521233216802.png)
...
18. 如果直接对上述结果进行绘制，就会出现如下锯齿：
19. ![image-20210522015426895](计算机图形学和OpenGL.assets/image-20210522015426895.png)

# 反走样

1. 抗锯齿=反走样=antialiasing。
2. 摩尔纹，将左图的奇数行和列都去除掉。属于空间的欠采样。
3. ![image-20210522102952477](计算机图形学和OpenGL.assets/image-20210522102952477.png)
4. 车轮效应是时间上的欠采样。
5. 欠采样就是信号的变化太快，采样速率太慢了。
6. blurred aliasing：先将边界突兀的三角形模糊化，再进行采样。模糊就相当于低通滤波了。如果先采样，再模糊则达不到反走样的效果。
7. ![image-20210522103429001](计算机图形学和OpenGL.assets/image-20210522103429001.png)
8. 使用下图中的采样频率对蓝色和黑色的曲线进行采样，会得到相同的离散点。
9. 走样（混叠）：两种不同频率的信号的采样结果相同。
10. ![image-20210522104429281](计算机图形学和OpenGL.assets/image-20210522104429281.png)
11. 图像的频率，像素值在空间的变化快慢。
12. <img src="计算机图形学和OpenGL.assets/image-20210522104857066.png" alt="image-20210522104857066" style="zoom:50%;" />
13. 高通滤波后的结果：
14. <img src="计算机图形学和OpenGL.assets/image-20210522105024618.png" alt="image-20210522105024618" style="zoom:50%;" />
15. 低通滤波后的结果：
16. <img src="计算机图形学和OpenGL.assets/image-20210522105311126.png" alt="image-20210522105311126" style="zoom:50%;" />
17. 卷积定理：时域（空间域）上的信号卷积等价于频域中的信号傅里叶变换的乘积。
18. 对下面的这幅图进行卷积（就是加权平均）计算，该滤波器主要在低频部分值较大，是低通滤波。如果卷积核扩大，编程9x9的，那么低通滤波器的截止频率会降低。极限的情况，1x1的卷积核，相当于没有滤波，即截止频率无限大。
19. <img src="计算机图形学和OpenGL.assets/image-20210522105700719.png" alt="image-20210522105700719" style="zoom:50%;" />
20. 采样就是重复频域上的内容：采样需要时域（空间域）上的离散函数和被采样函数乘积，相当于频域上傅里叶变换的卷积。采样函数的傅里叶变换是一个冲激串。
21. ![image-20210522110350985](计算机图形学和OpenGL.assets/image-20210522110350985.png)
22. 采样频率决定了卷积核的间隔，如果采样频率越低，卷积核间隔就过小，则会造成频域上的混叠。
23. 奈奎斯特定理：采样频率>=2信号最高频率，才不会引起混叠。
24. ![image-20210522110549666](计算机图形学和OpenGL.assets/image-20210522110549666.png)
25. 增加采样率，也就是使用像素点更密集的屏幕，可以减少走样。但是通常说的反走样是在同样的屏幕上进行的。
26. 模糊操作就是进行低通滤波，这样所需的最小采样频率就降低了。
27. ![image-20210522111057921](计算机图形学和OpenGL.assets/image-20210522111057921.png)
28. 
29. MSAA 多重采样反走样：考虑每一个像素内部的亚像素，例如将一个像素分为4x4。然后分别计算这16个亚像素的中心是否在三角形内部，从而计算出每个亚像素中心的取值。然后将这16个亚像素进行平均，就得到整个像素的值。
30. 该方法只是进行反走样的第一步，即模糊操作。
31. <img src="计算机图形学和OpenGL.assets/image-20210522112028796.png" alt="image-20210522112028796" style="zoom:50%;" />
32. 正常采样的结果：
33. <img src="计算机图形学和OpenGL.assets/image-20210522112312404.png" alt="image-20210522112312404" style="zoom:50%;" />
34. 进行2x2多重采样：
35. ![image-20210522112509084](计算机图形学和OpenGL.assets/image-20210522112509084.png)
36. 多重采样平均后的结果。
37. ![image-20210522112538899](计算机图形学和OpenGL.assets/image-20210522112538899.png)
38. 每个像素模糊后的结果。在此基础上采样就是它本身。
39. ![image-20210522112732455](计算机图形学和OpenGL.assets/image-20210522112732455.png)
40. 其他常用的抗锯齿方案：FXAA，TAA。
41. 超分辨率：将一张小图变大，但是又尽可能地消除锯齿。

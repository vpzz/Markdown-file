...
3. ![image-20210513225059830](汇编语言.assets/image-20210513225059830.png)
...
5. ![image-20210515235307060](汇编语言.assets/image-20210515235307060.png)
...
7. ![image-20210516005038257](汇编语言.assets/image-20210516005038257.png)

8. ebp指向0x00bafb18，ebp内存储的是上一个函数的栈底（是有当前函数第一个指令 push ebp 压入栈的），ebp+4为程序的返回地址，ebp+8为第一个参数，ebp+12为第二个参数等等。

9. ![image-20210516005615654](汇编语言.assets/image-20210516005615654.png)

10. 函数执行完毕，堆栈平衡，eax发生了变化，存储着返回值，栈的增长方向多了一些垃圾（参数，缓冲区等）。

11. 如果函数没有return内容，那么写不写return都是一样的。

12. ![image-20210516151532084](汇编语言.assets/image-20210516151532084.png)
...
26. ![image-20210530232230994](汇编语言.assets/image-20210530232230994.png)

27. 对于一个4位2进制数1111，加1之后的结果为10000，产生进位存储不下了，CF置为1。
...
94. 控制寄存器（CR0～CR3）用于控制和确定处理器的操作模式以及当前执行任务的特性。32位下寄存器为32位，第0位PE(Protected-Mode Enable)为0表示CPU处于实模式下，为1表示CPU处于保护模式下，并使用分段机制。第31位PG(Paging Enable)为0表示启动了分页机制，为1表示没有启动。

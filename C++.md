...
5. 在C++中，等号可以进行初始化和赋值，但是他们是完全不同的两个操作。在很多程序语言中，二者的区别不重要，在C++中有时也不重要，但是不能混用。
...
13. 容器一般搭配迭代器使用。一般的智能指针只用处理*和->运算，而迭代器智能指针还需要处理++和--等普通指针可以进行的运算。
...
11. 数组名可以看做是一级指针，但是数组名的地址并不是二级指针。因为数组名并不是一个变量，没有地址：
...
12. 指向数组元素的指针可以使用迭代器支持的所有操作。C++11引入了两个全局函数begin和end，定义在iterator头文件中。接受数组名为参数，分别返回首元素指针和尾后指针。

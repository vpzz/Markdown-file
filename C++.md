...
16. 大多数系统中，利用main函数的返回值来确定程序运行是否成功，一般返回0表示成功。

17. C++源文件常见的后缀名有.cc .cxx .cpp .cp .C。不同编译器的约定不一样。头文件的后缀名常见的有.H .hpp .hxx等。

18. GNU的C++编译器是g++。MSVC的C++编译器是cl。

19. #include和头文件的名字必须出现在同一行。通常情况下#include语句出现在所有函数和类定义之外。可以在需要的时候再include，而不是将其都放在开头。

20. 注释用来概述算法，指明变量的用途，解释难懂的步骤。错误的注释比没有注释更可怕，因此更新代码时，记得同步更新注释。C++有两种注释，界定符注释不能嵌套，因此如果要注释代码，建议用单行注释。

    ```c++
    //单行注释，以//开始，以换行符结束，可以包含任何文本，包括额外的双斜线，除了换行符。
    
    //界定符注释，继承自C语言，以/*开始，以*/结束。可以包含除了*/以外的任何内容，包括换行符。这种注释可以跨越多行，为了更清晰地表示中间行都是注释，可以在中间行的开头加上*，一般会让*对齐。
...
21. 语句块就是用花括号包围的零或多条语句的序列。语句块也是语句的一种，在任何要求使用语句的地方都可以使用语句块。

22. 循环，分支语句头中定义的变量只在其内部有效，例如：
...
23. 编译器只能检查形式上的错误，而不能检查出逻辑上的错误。常见的编译器错误有：

    1. 语法错误，例如括号不匹配，少了分号等。

    2. 类型错误，实参和形参类型不相容，赋值两侧类型不相容等。

    3. 声明错误，变量没有先声明就使用了。标准库的名字没有加std::，名字拼写错误。

24. 一个好的习惯是从前往后看错误提示，修改一个就编译一下。

25. C++ 程序是自由格式free form的，左花括号可以放在上一行末尾，也可以放在下一行开头。不过左花括号必须是形参列表后第一个非空，非注释的字符。

27. string和vector是最重要的两种标准库类型，前者支持可变长的字符串，后者支持可变长的序列。他们都是对内置数组的抽象，更方便使用。
...
1. 类类型变量如果未指定初值，则按照类定义的方式进行初始化。
2. C++是中静态类型的语言，也就是说类型检查发生在编译时，编译器必须知道每个变量对应的数据类型，从而为其分配内存。
3. C++的强大体现在他对于自定义数据结构的支持。C++语言的一个设计目标就是让类类型和内置类型一样好用。
4. C++定义的类型分为算术类型和空类型（void）。
...
5. 算术类型有：bool、char、wchat_t、char16_t、char32_t、shrot、int、long、long long、float、double、long double。
6. C++提供了四种字符类型，基本的是char，确保可以放下机器的基本字符集的任意一个字符，和一个机器字节相同。wchar_t确保可以放下机器扩展字符集的任意一个字符。char16_t和char32_t表示Unicode字符集中的字符。后三种是扩展的字符类型，都是无符号的。
7. C++规定，int至少和short一样大，long至少和int一样大，long long至少和long一样大。其中long long是C++11新定义的。
8. 可寻址的最小内存块称为字节byte，存储的基本单元称为字word，通常由几个字节组成。只有内存地址是不够的，还需要知道类型，从而知道一共有多少个字节，如何解析这些数据。
9. long double称为扩展精度。C++为每个类型指定了有效数字的下限。通常来说，编译器会将float实现为32位的，具有7个有效数字。double为64位的，具有16个有效数字。而long double则为96或128位，有效数字不定。标准规定long double的精度不低于double。
10. unsigned int可以缩写为unsigned。字符型被分为了三种：char、signed char、unsigned char。类型char会表现为后两种中的一种，具体有编译器决定。为了保险起见，如果需要一个不大的整数，应明确说明是否带符号。
11. C++标准并没有规定带符号类型应该如何存储，但是约定了表示范围内的正数和负数的量应该平衡。因此8bit的带符号整型理论上应该为-127→+127。不过实际实现为-128→+127。
12. 类型选用建议：
    1. 优先使用int进行整型运算，short太小，而long一般又和int一样大。如果数值超过了int的范围，可以使用long long。

    2. 不要使用char和bool类型数据进行算术运算，只是在存放字符或布尔值时才是用他们。

    3. 执行浮点运算优先选用double，因为float的精度往往不够，而且执行单精度和双精度浮点运算的性能消耗几乎一样。对于某些机器来说，double甚至还更快。long double提供的精度一般是用不到的，其增加的消耗却不可忽视。

13. 未定义的行为意味着程序可能继续工作，可能崩溃或产生垃圾数据。它来源于编译器无需（有时候无法）检查的错误。在某些情况下，包含未定义行为的程序仍然会正常工作，但是换一个编译器编译，程序就可能报错。这种bug难以发现。
14. 程序应该避免依赖环境的行为，例如，如果把int看做是恒定不变的32位大小，那么这种程序就是不可移植的。

## 整型和浮点型

1. 一个算术表达式中，既有无符号数，又有int时，int就会转换为无符号数。把int转换为无符号数的过程和将int值赋给无符号类型是一样的。
...
2. 当从一个无符号数中减去一个值时，不论这个值是不是无符号数，都不必须确保结果不能是一个负值。实际上这个值如果是有符号，会被转为无符号数。无符号数的算术运算结果还是无符号数（取余即可）。无符号数是可以做减法的。
...
4. 一个形如42的值被称为字面值常量。每个字面值常量都对应一种数据类型，他的形式和值决定了其数据类型。

5. 可以将整型字面值写成10、2（以0b或0B开头）、8（以0开头）、16（以0x或0X开头）进制等形式。

6. 默认情况下，十进制字面值是带符号数，八进制和十六进制字面值可能是带符号，也可能是无符号数。

   1. 十进制字面值的类型是int、long、long long中能容纳数值的最小的那个类型。
   2. 二进制，八进制，十六进制字面值的类型是int、unsigned int、long、unsigned long、long long、unsigned long long中能容纳数值的最小的那个类型。上述类型从左往右依次进行适配。
   3. 如果最大类型都放不下这个字面值，则会编译错误。short没有对应的字面值。

7. 尽管整型字面值可以存储在带符号类型中，严格来说，十进制字面值不会是负数，例如使用了-42这个负的十进制字面值，负号并不在字面值之内，他的作用仅仅是对字面值42取负值而已。

8. 浮点型字面值表现为小数或科学计数法表示的指数。指数部分可以用e或E。默认情况下，浮点型字面值是double类型。
...
1. 由单引号包括起来的一个字符为char型字面值。双引号包括起来的零个或多个字符构成字符串型字面值。字符串字面值的类型实际上是有字符常量构成的数组。编译器在每个字符串的结尾处添加一个空字符'\0'。因此字符串字面值占据的空间要比是自己的字符多1个字节。例如"abc"的类型是const char [4]。

2. 如果两个字符串字面值的位置紧邻，且仅由空格，缩进，换行符分隔，则他们实际上就是一个整体。

   ```c++
   "abc"  "def"   //等价于 "abcdef"。前一个字符串的'\0'会被去掉。
   cout << "abc"  //如果一行写不下，可以分开书写。
       "def"
   ```

3. 有两类字符程序员不能直接使用：不可打印的字符，例如退格或其他控制字符，因为他们没有可视的图符；C++语言中有特殊含义的字符，例如单/双引号，问号，反斜线。这两种情况下，需要使用到转义序列：

   ```c++
   // C++规定的转义序列有如下：
     \n       \t       \a       \v        \b     \"     \\     \?     \'     \r     \f
   换行符  横向制表符  响铃符  纵向制表符  退格符  双引号  反斜线  问号  单引号  回车符  进纸符(换页)
   ```

4. 在程序中，转义字符会被当做一个字符使用，可以将转义序列和普通字符一样使用，融入到字符串中。也可以使用泛化的转义序列，形式为\x后面跟1个或多个16进制数字；或\后面跟1、2、3个八进制数字。数字部分是字符集中字符对应的数值。
...
5. 如果\后面跟着的8进制数超过3个，则只有前三个和\构成转义序列。相反，\x会和后面的所有十六进制数字构成转义序列。

   ```c++
   "\1234"   //实际上是2个字符 '\123' 和'4'。
   "\x1234"  //只是1个字符，这表示一个16位的字符。超过8位的16进制字符需要配合扩展字符类型来使用。
   ```

6. 通过给字面值添加前缀或后缀，可以改变他们的默认类型：
   ...
   21L    //有符号整数，实际为long
   21LL   //有符号整数，实际为long long
   21UL   //无符号整数，实际为unsigned long
   21ULL  //无符号整数，实际为unsigned long long
   //浮点型字面值
...
7. 对于整型字面值来说，能够分别指定它是否带符号和所占的空间大小。后缀中有u或U，则属于无符号，只能从unsigned int，unsigned long，unsigned long long中进行匹配。如果后缀中有l或L，则在long，unsigned long，long long，unsigned long long中匹配；如果后缀中有ll或LL，则在long long，unsigned long long中匹配。因此后缀一共有5种：U，L，LL，UL，ULL。这里推荐全用大写，因为小写的l和数字1很像，容易认错。
...
1. 类型所能表示的范围决定了类型转换的过程：
   ...
   1. 把一个非bool的算数值赋值给bool类型，0的结果为false，其余的都是true。
   2. 把bool值赋值给非布尔类型，false的结果为0，true的结果为1。
   3. 把浮点数赋值给整型，数值进行了取整，小数部分被去掉了。
   4. 把整数赋值给浮点类型，小数部分记为0，如果整数的位数过长，超过了浮点数的有效数字位数，会有精度损失。
   5. 把一个超过其范围的值赋给无符号类型时，结果是改值对无符号类型总数取模后的结果。例如将-1赋值给unsigned char，后者表示范围为0→255，一共256个数，因此结果为-1/256=-1……255。这也是溢出的原理。
   6. 把一个超过其范围的值赋给带符号类型时，结果是未定义的。

2. 上面的类型转换都是静态，隐式的，在编译阶段由编译器完成的。

3. 算术类型之间的隐式转换被设计为尽可能避免损失精度。因此整数会转换成浮点数。
...
5. 对于bool，unsigned char，char，signed char，short，unsigned short等类型来说，只要他们所有可能的值都能存在int内，就会提升为int，否则提升为unsigned int。
...
6. 较大的char类型（wchar_t，char16_t，char32_t）提升为int，unsigned int，long，unsigned long，long long，unsigned long long中能容纳原类型所有可能值的最小类型。

7. 如果某个运算对象是无符号类型，那么转换的结果依赖于机器中各个整数类型的相对大小。
...
8. 上面说的类型大小不是所占空间的多少，而是根据 char，short，int，long，long long来排序的。

9. 可以看出，算术类型的隐式转换的规则特别复杂，如果可以还是使用显式类型转换，这样可以避免很多错误。

10. 除了算术类型的隐式转换以外，还存在以下几种隐式类型转换：
...
11. 虽然有时候不得不使用强制类型转换，但是这种方法是非常危险的。

12. 命名强制类型的转换形式为cast-name\<type\>(expr)，如果type为引用，则结果为左值，cast-name有4种：
...
13. 只有const_cast可以改变表达式的常量属性。使用其他类型的cast都会引发编译错误。同样，const_cast也不能用来改变表达式的类型。
...
1. 变量在创建的同时获得一个特定的值，这个过程被称为初始化。可以用任意复杂的表达式来初始化一个变量。

   ```c++
   int sum =0,value;  //定义2个int类型的变量，并未第一个变量赋初值。
   int a = 1,b = a+1; //正确，先定义并初始化a，然后定义b，并用a+1初始化b。
   std::string book("test");  //定义了一个std::string类型的对象，用字符串字面值初始化。
   ```

2. 在C++中，等号可以进行初始化和赋值，但是他们是完全不同的两个操作，后者是把变量的当前值擦除，用一个新的值代替，并不用分配内存空间。

3. C++语言定义了好几种不同的初始化方法：

   ```c++
   //以下四种都可以。
   int a = 0;
   int a = {0};  //列表初始化，这是C++11提出的。也可以用这个花括号的方法来赋值。
   int a{0};     //也是列表初始化。
   int a(0);
   ```

4. 以上几种初始化大部分情况下可以互相等价使用，但是存在例外：
...
5. 将列表初始化方法用于内置类型变量时，如果初始值存在丢失风险，编译器会报错。

   ```c++
   long double ld = 3.1415926536;
   int a{ld},b={ld}; //错误，转换未执行，因为存在丢失信息的风险。小数部分可能会丢失，同时数值可能超过int的范围。
   int c(10),d=ld;   //正确，转换执行，确实丢失了部分值。
   ```

6. 如果定义变量时没有指定初始值，则变量会被默认初始化，此时变量被赋予了默认值。默认值由变量的类型决定，同时定义变量的位置也会影响默认值。

   1. 如果内置类型的变量未被显式初始化，则它的值由定义位置决定。如果定义在任何函数体之外则被初始化为0,；如果定义在函数体之内，则不被初始化。一个未被初始化的内置类型变量的值是未定义的。使用未初始化的变量是造成很多bug的原因。

   2. 每个类各自决定其初始化对象的方法。而且是否允许不经初始化就定义对象也由类自己决定。绝大多数类都支持无需显式初始化而定义对象。这样的类提供一个合适的默认值。
...
7. 为了允许把程序分成几个部分编写，C++支持分离式编译机制，每个源文件可以独立编译。这就需要有在文件之间共享代码的方法。

8. 为了支持分离式编译，C++将声明和定义分离开来。

   1. 声明使得一个名字为程序所知，一个文件如果想使用别处定义的名字，则必须包含对那个名字的声明。
   2. 定义则创建于名字关联的实体。

9. 声明和定义都规定了变量的类型和名字。不同点是定义还申请了内存空间，也可能为变量赋初值。定义是包含声明的。

10. 如果想要声明而非定义一个变量，应该在变量名前添加extern，而且不要显式初始化它。任何包含了显式初始化的声明就会变成定义。
...
11. 如果要在多个文件之间使用同一个变量，就需要将声明和定义严格区分开。变量能且只能被定义一次，但可以被声明很多次。变量的定义只能出现在一个文件中，其他文件如果需要用到该变量，声明即可，但是不能重复定义。

12. C++的类型检查，要求使用任何变量前，至少要声明。编译阶段可以只有声明，具体的变量内存可以在链接阶段再寻找。

13. C++为标准库保留了一些名字，用户自定义的标识符应该满足以下要求：
...
14. 关于标识符的约定：
    ...
    2. 变量名一般用小写，用户自定义的类名一般大写字母开头。标准库的所有标识符都是全小写。

    3. 如果标识符以多个单词组成，单词间要有明显的区分。用下划线或驼峰。student_index或studentIndex。

15. C++中大多数作用于都用{ }进行分割。同一个名字在不同的作用域中可能指向不同的实体。名字的有效区域开始于声明语句处，结束于声明语句所在的作用域末端。

16. 定义在所有函数体之外的名字具有全局作用域。一旦声明以后，在程序的任何位置都可以使用。

17. 一般来说，在对象第一次使用前再进行定义是一个比较好的选择，这样更容易找到变量的定义，同时也方便赋一个合理的初值。

18. 作用域能嵌套，但是不能交叉。作用域内一旦声明了某个名字，则其内层的所有作用域都能访问该名字。允许在内层作用域中重新定义外层作用域中已有的名字。外层名字会被暂时隐藏。
...
1. 复合类型是指基于其他类型定义的类型，例如引用，指针和数组。一条声明语句由一个基本数据类型和紧随其后的一个声明符列表组成，每个声明符命名了一个变量并指明该变量为与基本数据类型有关的某种类型，简单声明语句中声明符就是变量名，此时变量的类型就是声明的基本数据类型。

2. 类型修饰符是声明符的一部分，int \*p和int\* p等价，建议使用前一种，不容易产生误解。

   ```c++
   int* a,b;  //实际上是定义了一个int*指针a,一个int变量b。
   ```


## 引用

1. 引用为对象起了一个别名，引用类型引用另外一种类型，引用并非对象，而是为一个已经存在的对象起的另一个名字。引用的初始化并不是拷贝变量的值，而是将名字绑定到初始值上。一旦初始化完成，就不能改变引用绑定的对象，因此引用必须初始化。

   ```c++
   int a = 3,b = 5;
   int &ref = a;
   int &ref1;        //错误，引用必须被初始化。
   ref = b;          //为引用赋值，并非改变引用绑定的对象，而是为引用绑定的对象赋值。
   int c = ref;      //以引用为初始值，实际以引用绑定的对象为初始值。
   int &ref3 = ref;  //由于引用并非对象，因此无法定义引用的引用，所以这句是将ref3和ref绑定的同一个对象上。
   ```

2. C++11中新增了一种引用，右值引用。主要用于内置类，默认情况下，引用指的就是左值引用。
...
4. 除了const和多态需要以外，引用和指针的类型都要和绑定的对象严格匹配。 不能将int&绑定到一个double对象上，也不能将一个int*指针指向double对象。


## 指针

1. 指针和引用都实现了对对象的间接访问，不同的是：指针本身是一个对象，指针可以先后指向不同的对象；指针无需在定义时赋初值。和其他内置类型一样，定义在函数内的指针不会被默认初始化。

2. 引用不是对象，没有实际地址，因此无法定义指向引用的指针。

3. 使用取地址符&获取对象的地址，使用解引用符*来访问对象。解引用操作只适用于确实指向了某个对象的有效指针。

4. 空指针不指向任何对象，在试图使用指针前，应该检查其是否为空。推荐使用nullptr，避免使用NULL。

   ```c++
   //生成空指针的方法：
   int *p1 = nullptr;  //C++11标准增加的。特殊类型的字面值，可以被转换成任意其他指针类型
   int *p2 = 0;
   int p3 = 0;
...
5. 访问未初始化肯能会出错，也可能不会出错，这种bug很难发现。因此建议在定义时进行初始化，如果不知道初始化为何值，可以先初始化为空指针。

6. 当且仅当指针为空时，其条件值为false。对于两个相同类型的合法指针，可以用==或!=进行运算。指针相等意味着指向同一个对象。

7. void*是一种特殊类型的指针，可以用于存放任意对象的地址。不能直接操作void\*指针，因为我们不知道对象是什么类型的。用途：

   1. 拿它和其他指针比较
   2. 作为函数参数和返回值的类型
   3. 赋值给另外一个void*的指针

8. 声明符中的修饰符个数并没有限制。可以定义指针的引用。
   ...
   //复杂类型的阅读应从左往右进行。先从离变量名最近的开始，&r表明r是一个引用，然后有一个*表明是这个引用绑定在指针上，最后是int。
   ```
...
3. const和非const对象可以互相初始化，因为拷贝一个对象并不会改变他的值。
...
4. 默认情况下，const对象仅在文件内有效。

   1. 对于编译时进行初始化的const对象，编译器会找到所有使用到该对象的地方，用初始值替换。要做到这一点，就必须在所有用到const对象的文件中都有对它的定义。同时又为了避免重复定义，所以const对象被设定为仅在文件内有效。多个文件中出现同名的const变量时，互相独立。
   ...
   2. 对于运行时初始化的const对象，如果需要在文件间共享（不希望编译器为每个文件单独生成一个），可以对该变量的声明和定义都添加extern，这样只需定义一次就行了。
      ...
      ```c++
      //a.cpp
      extern const int a = 6;  //定义并初始化
      //b.cpp
      extern const int a ;  //仅声明，extern表明这个变量并非本文件所独有，它的定义出现在别处
      int main() {
      	cout << a<<endl;  //结果为6。
      }
      ```
...
1. 对常量的引用：绑定到常量对象的引用。不能通过引用来改变对象的值。简称为==常量引用==，严格来说并不存在常量引用，因为引用本身不是一个对象，更不能让他本身恒定不变。实际上C++不允许引用重新绑定到新对象上，从这个意义上来说，所有的引用都是常量的。

   ```c++
   const int a = 4;
   const int &r = a;
   r = 5;
   int r2 = a;  //不能将一个常量对象绑定到非常量引用上。如果可以的话，就能通过r2修改a的值了，产生了漏洞。
   ```
...
3. 允许使用任意表达式初始化常量引用，只要该表达式能够转化为引用的类型即可。
...
4. 编译器会将引用绑定到表达式对应的临时量对象上。临时量对象就是：编译器需要一个空间来暂存表达式的求值结果时，临时创建的一个未命名的对象。

   ```c++
   double dval = 3.14;
   const int &r5 = dval;  //正确
   //为了确保r5能够绑定一个整数，编译器会把上述代码变成：
   double dval = 3.14;
   const int tmp = dval; //产生一个临时量对象
   const int &r5 = tmp;  //让r5绑定到这个临时量对象上。
   
   int & r4 = r1 * 2;  // 错误，因为会将r4绑定到一个临时量对象上去，而非常量引用一般是想要修改对象内容的。因为大家不会想要修改一个临时量对象的值，因此C++将这一行为定义为非法。
   ```

5. 常量引用仅对引用本身可以参与的运算做了限定，但是引用的对象本身不一定要是一个常量。允许通过其他途径改变对象，这只是引用自以为是，自觉不去修改。
...
3. 类似于常量引用，指向常量的指针不能改变其所指对象的值，但是简称为常量指针（pointer to const）。只能使用常量指针指向常量对象。

   ```c++
   const int a =3;
   int *p1 = &a;  //错误，如果允许的话，就可以通过p1修改a了，产生漏洞。
   const int *p2 = &a; //正确，也可以不初始化。
...
5. 指针常量（const pointer）：允许把指针本身定义为const，这样就不能改变它的指向。和其他常量对象一样，定义时必须初始化。

   ```c++
   int a = 3,b =5;
   int * const p = &a;  //指针常量，将const放在p前面，意味着p本身是一个常量。
   p = &b;   //错误
   *p = 7;   //正确，可以通过p修改指向的对象。
   ```
...
7. 为了方便区分，可以用顶层const表示指针本身是一个常量，底层const表示指向的对象是一个常量。顶层是直接的，底层是间接的。一般的，顶层const可以表示任意的对象是常量，而底层const只能用于指针和引用。指针可以是顶层的，也可以底层的，也可以二者都是。而引用一定是顶层的，可以是底层的。
...
4. 只有算术类型，引用，指针能定义常量表达式，类类型不能。constexpr指针的初始值只能是nullptr/0或某个存储于固定地址的对象。函数内部定义的变量都是存储在栈上，地址不固定。定义于所有函数外边的变量的地址固定不变，定义在函数内部的static变量也具有固定地址，可以用来初始化constexpr指针。constexpr的引用也只能绑定到具有固定地址的对象上。
...
1. 表达式是有运算对象和运算符组合而成。表达式的求值产生一个结果。字面值和变量是最简单的表达式，其结果就是字面值和变量的值。复合表达式是指含有多个运算符的表达式，表达式可以分为子表达式。

2. 三元运算符有三个运算对象，函数调用也是一种特殊的运算符，运算对象数量不限。在源文件中用" "包括起来的C风格字符串称为 字符串字面值常量。

3. 要理解表达式的含义，首先要理解运算符的优先级，结合律，求值顺序。优先级和结合律决定了运算对象的组合方式。

4. 高优先级的运算符比低优先级的运算符更为紧密地组合运算对象。算术运算符满足左结合律，意味着如果优先级相同，将按照从左往右的顺序组合。
   ...
   //根据运算符的结合律
   5-3+2; //等价于(5-3)+2
   cout << a << b;     //优先级相同，<<满足做结合律，因此等价于  (cout << a )<< b
   ```

5. 优先级规定了运算对象的组合方式，但是并没有规定运算对象按照什么顺序求值，大多数情况下也不会明确指定。对于那些没有明确指定执行顺序的运算符来说，如果表达式指向并修改了同一个对象，那么将会引发错误的并未定义的行为。

   ```c++
   int i = f1() * f2(); //可以肯定的是 f1和f2 一定会在乘法进行前调用，但是无法确定那个函数会先执行。
   
   int i = 0;
   cout << i << " " << ++i << endl;  //未定义，因为编译器可能先求右边的++i的值，再求左边i的值，此时结果1 1；也可能先求左边i的值，再求右边++i的值，结果为0 1。
   ```

6. 有4种运算符明确规定了运算对象的求值顺序：
...
7. 运算对象的求值顺序和优先级，结合律无关。例如：

   ```c++
   f()+g()*h()+j();
   //优先级规定g()的返回值和h()的返回值相乘
   //结合律规定f()的返回值和乘积先加，然后再和j()的返回值相加。
   //但是并没有规定这些函数的调用顺序，如果这几个函数不改变同一个对象的状态，也不执行IO任务，则先后顺序无所谓，否则应该将他们拆开成多个表达式。
   ```

8. 一般情况下，运算对象的可以按任意顺序进行求值，但是如果一条子表达式中改变了某个运算对象的值，而另一个子表达式又要用到该运算对象的值，则会引发未定义行为：

   ```c++
   string s("abcd efg");
   //将字符串s中的第一个单词改成大写形式。
   for (auto it = s.begin(); it != s.end() && !isspace(*it); ++it){ //将解引用it和递增it放在两个表达式中，结果不会受求值顺序影响
       *it = toupper(*it);
   }
   auto beg = s.begin();
   while(beg != s.end() && !isspace(*beg)){
       *beg = toupper(*beg++);  //赋值运算符的左右两端都用到了beg，而且右端还更改了beg的值。如果先执行左侧，等价于*beg = toupper(*beg);beg++;  如果先执行右侧，等价于*(beg+1) = toupper(*beg);beg++; VS2022是先对右侧求值。先对右侧求值会导致结果不是string("ABCD efg")。同时会对尾后位置解引用。
   }
   ```

9. 算术运算符有7个，+ - * / % + -，%是取余，最后是一元正号和一元负号。算术运算符的运算对象和求值结果都是右值。算术运算符都能作用于所有的算术类型，同时一元正号，加法，减法还可以作用于指针。

10. 一元正号作用于一个指针或算术值时，返回运算对象值提升后的一个副本；一元负号作用于算术值时，返回结果是提升后的：
...
11. 整数相除的结果还是整数，小数部分会被舍去。C++语言的早期版本允许结果为负值的商向上或向下取整，C++11标准中规定一律向0取整，也就是直接取出小数部分。取余运算的运算对象必须是整数。
...
12. 取余运算需要满足：$(m/n)*n+m%n$​的结果和m相等。也就是说如果m%n不等于0，那么他的符号和m的相同。C++的早期版本允许m%n的符号匹配n的符号，且商向负无穷一侧取整。不过这一做法在新标准中已经被禁止了。除了-m导致溢出的情况，其他时候都满足下面的式子：
...
13. 关系运算符作用于算术和指针类型，有 <  <=  >  >=  ==  !=；逻辑运算符作用于任意能转化为bool值的类型，有!  &&  ||。关系运算和逻辑运算的结果都是bool类型的右值。

14. 关系运算符返回值为bool值，连用的话会引发意外：
...
15. 可以将一个算术对象或指针当做条件，来测试他们的真值。这种方法比和true比较要好：
...
16. 赋值运算符的结果是他的左侧运算对象，仍然是一个左值。如果左右两侧运算对象的类型不同，会先将右侧的转换为左侧的。
...
17. C++11标准允许使用花括号括起来的初始值列表作为赋值运算符的右侧对象。对于类类型来说，赋值细节由类来定义，vector模板重载了赋值运算符，==会调用拷贝赋值函数，是不是只适用于单个对象的拷贝，还是也适用于初始化列表？==
...
20. C++允许使用相等运算符作为条件，但是可能会带来意外，有的编译器会提示这点。常见的错误是使用=代替\==进行比较运算。

21. 每种算术和位运算符都有其复合赋值的形式。按位求反运算符没有复合赋值形式。
...
22. 递增，递减运算符可以应用于迭代器，因为很多迭代器对于算术运算是不支持的。这两种运算符分为前置和后置版本，必须作用于左值，前置版本返回左值，后置版本返回右值。
...
23. 除非必须，否则不用后置版本的运算符，因为后置版本要保存一个临时值用于返回，前置版本可以避免不必要的工作。对于整数和指针来说，编译器可能对这种额外的工作进行优化，但是对于相对复杂的迭代器类型，这种额外的工作就比较大了。

24. 在一个表达式中，递增变量，并使用原来的值，可以用后置运算符。后置递增运算符的优先级高于解引用符，因此不用加括号。
...
25. 箭头运算符作用于指针或迭代器时，结果是左值。点运算符作用于左值时，结果是左值；作用于右值时，结果是右值。
...
26. 条件运算符允许将简单的if-else语句嵌入到单个表达式中。当条件运算符的后两个表达式都是左值或能转换成同一种左值类型时，结果也是左值，否则，结果为右值。条件运算符的优先级特别低，因此一般都要加括号：
...
27. 位运算符作用于整数类型的运算对象，把运算对象看做二进制位的集合，也就是位串。提供检查和设置二进制位的功能。标准库bitset类型支持任意大小的二进制位集合，也可以使用位运算符。位运算符都满足左结合律。运算对象可以是带符号的，也可以是无符号的。如果运算对象是有符号的，且是负的，那么位运算符如何处理符号位依赖于机器，此时左移可能会改变符号位，因此是一种未定义的行为。因此强烈建议仅将位运算作用于无符号数。
...
28. 移位运算要求右侧运算对象不能为负数，而且该值必须小于移位运算结果的位数，否则将产生未定义的行为。移位运算移出边界的位会被舍去。
...
29. 左移运算符就是在右侧插入0，右移运算符的行为依赖于左侧运算对象的类型，如果是无符号数，则插入0，如果是有符号数，插入符号位或0,，具体行为依赖于环境。

30. 可以通过位运算来设置，清除，查询指定的位的状态：
...
31. 移位运算符满足左结合律。优先级不高不低，比算术运算符低，比关系，赋值，条件运算符高。因此用来进行IO时，建议多加括号。

32. sizeof运算符返回表达式或一个类型名所占的字节数。结果为size_t类型的常量表达式。sizeof作用于表达式时返回的是结果类型的大小，但并不计算对象的值。两种使用方法，加不加括号都行：
...
33. 逗号运算符，有两个运算对象，规定了运算对象求值的顺序。先求左侧的，把值丢弃，再求右侧的值，作为表达式的结果。如果右侧运算对象是左值，最终结果也是左值。经常用在for循环内：
...
34. 一般运算符要求运算对象的类型相同，或者能够转化成同一个类型也可以。

35. C++的表达式只能是左值或右值。这两个名字都是从C语言继承过来的，左值可以位于赋值语句的左侧。但是在C++语言中，区别更为复杂：

    1. 左值表达式的求职结果是一个对象或一个函数。
    2. 虽然某些表达式的求值结果是对象，但是他们是右值，例如常量对象不能作为赋值语句的左值。

36. 当一个对象被用作左值时，使用的是对象的身份，即内存的位置；被用作右值时，使用的是对象的值。

37. 除了右值引用以外，在需要右值的地方，都可以用左值替代，反之则不可以。

38. 常见的用到左值的运算符的地方：

    1. 赋值运算符要求非常量左值作为左侧运算对象，结果仍然是一个左值。
    2. 取地址符作用于一个左值对象，结果是一个右值指针。
    3. 内置解引用运算符，下标运算符，迭代器解引用运算符，string和vector的下标运算符的求值结果都是左值。
    4. 内置类型和迭代器的递增递减运算符作用域左值，前置版本所得的结果是左值。

39. decltype作用域左值（不是变量）时，得到的是一个引用：
    ...
    decltype(&p) p;      //等价于 int** p; 因为&p是右值。
    ```

40. 大部分的运算符都是左结合的，也就是说同一个优先级的运算符并列，会优先计算左边的运算，相当于给左边加了括号。赋值运算，复合赋值运算就是右结合的：
...
42. 使用点运算符.来获取类对象的成员；使用调用运算符()来调用函数，括号内为实参列表。

44. C语言标准库在iso646.h定义了一批常见运算符的可选拼写，这些宏可以帮助非QWERTY键盘的用户使用C语言的逻辑运算符：
...
45. and虽然不是C++的保留关键字，但是C++的保留替代标记。在c++语言中可以直接使用and。但是在C语言中需要包含iso646.h才可以。有的C++编译器不支持and，包含\<ciso646\>即可。
...
2. 类体最后必须用分号结束，这是因为类体后面可以紧跟变量名，用来直接定义该类的对象，不过不建议这么做。
...
3. C++11标准规定，可以为类的数据成员提供一个类内初始值。创建对象时，类内初始值将用于初始化该数据成员。没有初始值的成员会被默认初始化。
...
5. 可以将类定义在函数内，但是不推荐。一个类可能会被多个文件使用，为了使各个文件中的类保持一致，类通常被定义在头文件中，而且类所在的头文件的名字应该和类名一致。例如Sales_item类定义在Sales_item.h中。


## 类的定义

1. 成员函数可以在类中定义，也可以类内只写声明，在类外定义。在类外定义的时候要加上类名::，否则视为普通函数。在类内部使用类的成员不用限定符::，例如：
...
2. C++中函数分为两大类，一种是类的成员方法，必须在类内声明，但可以在类内或类外定义，另一种是普通函数。

3. 在类外定义的函数如果具有默认参数，只需在声明处写上默认参数即可，定义处不用也不能再写，否则会报错：
...
4. 普通函数例如操作符重载，声明写在.h文件中，定义写在.cpp中。如果都写在.h中会出现重定义，因为该类的cpp和main.cpp都会包含这个函数，这两个cpp文件各自编译，其内部都有该普通函数的实现，会造成冲突。如果非要将普通函数的实现写在.h中，可以加上inline避免重复定义。

5. 全局变量为了避免重复定义，都应只写在.cpp中，在其他类中使用该全局变量，使用extern int glint之类的语法说明。如果是只有一个.h的类。就可以都写在.h文件中。
...
6. C语言中使用struct定义结构体这种复合数据类型，使用该类型时还要加上struct关键字，为了方便可以使用typedef化简。在C++中则不用typedef，可以直接省略struct来将它当做一个类来使用。和class不同的是，如果不显式声明访问级别，struct的成员默认是public，class的成员默认是private。
...
1. 操作符重载本质上是函数重载，主要是方便使用。运算符重载，是为已经存在的运算符赋予了另外一层含义。能够改变运算对象和返回值的类型，但是不能修改运算对象的个数，运算符的优先级，结合律。
...
5. 复合可以嵌套，大小上叠加即可。
   ...
   <img src="C++.assets/image-20210815231531532.png" alt="image-20210815231531532" style="zoom:80%;" />
...
4. <img src="C++.assets/image-20220907204505865.png" alt="image-20220907204505865" style="zoom:80%;" />
...
16. ![image-20220907211330950](C++.assets/image-20220907211330950.png)
...
18. <img src="C++.assets/image-20220907214300118.png" alt="image-20220907214300118" style="zoom:80%;" />
...
14. <img src="C++.assets/image-20210817124346391.png" alt="image-20210817124346391" style="zoom:80%;" />
...
6. ![image-20210819213611196](C++.assets/image-20210819213611196.png)
...
1. 可以通过默认的方式初始化一个string对象，这样会得到一个空的string，其中没有任何字符。也可以提供一个字符串字面值，这样该字面值中除了最后的空字符以外的所有字符都会被拷贝到string对象中。string中不存在'\0'字符，size()更不会包括它。

   ```c++
   string s1;  //会调用默认构造函数，进行默认初始化
   string s2 = s1;  //调用拷贝构造函数。
   string s3(s1);   //同上
   string s4 = "hiya"; //调用转换构造函数，内容是 hiya 4个字符。
   string s5("hiya");  //同上。
   string s6(10,'c');  //内容是10个字符c。
   ```

2. 使用等号初始化一个变量，实际是进行拷贝初始化。如果不使用等号，则是直接初始化。拷贝初始化就是编译器将等号右侧的对象的值拷贝到正在创建的对象中。直接初始化实际上就是显式地调用构造函数（包括拷贝构造函数），编译器会根据参数匹配最优的。因此直接初始化可以有多个初始值。拷贝初始化只能有一个初始值。可以将拷贝初始化转化为对应的直接初始化，来判断调用的是哪个函数。
   ...
   //直接初始化也可能调用拷贝构造函数，例如s3；拷贝初始化可以不调用拷贝构造函数，例如s4。
   ```
...
3. 常见的操作：
   ...
   ```c++
   os << s;         //将s的内容输出到os中。
   is >> s;         //从输入流is中读取字符串赋值给s，会自动忽略开头的空白，从第一个真正的字符开始读取，知道遇见下一处空白结束。
   getline(is,s);   //从输入流is中获取一行字符。换行符也会被读取，但是不会存入string中。如果一开始就遇到换行符，则会返回空string。这个函数不会忽略任何空白。
   s.empty();       //如果为空，则返回true
   s.size();        //字符个数
   s[n];           //返回第n个字符的引用，char&类型，位置从0开始。
   s1+s2;          //字符串拼接。得到一个新的临时对象。
   s1 == s2        //如果包含的字符完全一样，则为true，大小写敏感。
   s1 < s2;        //根据字典顺序比较对应的字符，大小写敏感。如果两个string的长度不同，且较短对象的每个字符都和较长对象的对应字符相同，则较短对象<较长对象。
   ```

4. string类和大多数标准库类型一样，都定义了配套的类型，这些类型体现了标准库的机器无关性。例如string::size_type 就是string类中定义的一个类型。她不是成员变量，也不是成员函数，而是成员类型。不同的实现会使用具体的类型来模拟，例如vc下，string::size_type就是unsigned int。不过建议使用配套的类型，这样移植性好。

5. size成员函数返回值的类型为string::size_type类型。它是一种无符号类型，足以存放下任何string对象的大小。C++11中可以用auto或decltype来推断变量的类型：

   ```c++
   auto a = s.size();  //a是string:size_type类型的。
   ```

6. 字面值可以和字符串相加，左右都可以，但是两个字面值不可以相加。
   ...
   "abc" + "def" + string("hij");  //错误
   ```

7. 为了和C语言兼容，字符串字面值不是C++的标准库string类型的对象。

8. cctype头文件中定义了一组标准库函数用来处理单个字符。
...
9. 下标运算符接受的参数是string::size_type类型的值，可以传递一个有符号数，不过会被自动转换为无符号数。s[s.size()-1]是最后一个字符,当size等于0时，这个不成立，因为0-1的无符号数是一个特别大的数，会超出下标范围，产生out_of_range异常。

10. 如果s为空，s[0]的结果是未定义的。在每次使用下标前都要判断是否是合理的，string对象的下标必须>=0，<size。

    ```c++
    string s("abc");
    for (decltype(s.size()) index = 0; index != s.size(); index++){
        s[index];  //将index设置为size_type类型的，这样index总是>=0，然后只需让他不等于s.size()即可。
    }
    ```
...
2. C风格字符串不是一种类型，而是为了表达和使用字符串形成的一种约定的写法。一般用指针来操作这些字符串。头文件cstring中包含了一些操作函数：

   ```c++
   strlen(p);  //返回字符串的长度，不包含末尾的空字符。
   strcmp(p1,p2); //逐个比较两个字符串的元素，如果都相等，返回0。如果前面的大，返回正值，否则返回负值。
   strcat(p1,p2); //将p2附加到p1后面，返回p1。要求p1指向的内存是可写的，且有足够的空间。
   strcpy(p1,p2); //将p2拷贝给p1，返回p1。要求p1指向的内存是可写的，且有足够的空间。
   ```

3. 以上函数的指针必须指向以空字符作为结束的数组。

4. C风格字符串的比较和string的比较不同，要想逐个字符比较，应该用strcmp：
   ...
   x = p1 < p2;  //比较指针的值，而不是字符。结果为为未定义值，本次为false。
   ```
...
6. 如果程序中需要某个C风格字符串，不能直接使用string替代。
   ...
   string s("abc");
   char *str = s;  //错误
   const char *str = s.c_str();  //正确，不能省略const。因为c_str返回的是const char* 类型。无法保证c_str返回的值一直有效，事实上如果改变s，可能会改变使得str无效。
   ```

## vector

1. vector可以用来表示对象的集合，因此常被称为容器。其中存储的所有对象都必须是同类型的。
2. 引用不是对象，因此无法创建包含引用的vector。可以创建vector的vector。早期版本中，必须在外层vector的右尖括号和元素类型之间加一个空格，新版本则不用：
...
3. 定义和初始化vector：
   ...
   vector<T> v2(v1); //调用拷贝构造函数，等价于vector<T> v2 = v1;
   ...
   //如果使用了花括号形式，但是提供的值又不能用来列表初始化，则会尝试调用构造函数。以下只有s1是列表初始化。
   vector<string> s1{"hi"};   //一个元素，内容为hi。
   vector<string> s2("hi");   //错误，没有对应的构造函数。
   vector<string> s3{10};     //10个元素，都是空字符串，等价于vector<string> s3(10);
   vector<string> s4{10,"hi"};//10个元素，都是hi，等价于vector<string> s4(10,"hi" );
   ```
4. 要想使用列表初始化vector对象，花括号中的值必须与元素类型的构造函数相容。
5. 程序在运行时可以高效地向vector中添加元素，一般是先创建一个空vector，然后向其中添加元素。
6. 常用操作：

   ```c++
   v.empty();  //为空时返回true
   v.size();   //返回元素的个数，类型为vector<T>::size_type，而不是vector::size_type。
   v.push_back(t); //将元素t添加到v的尾端
   v[n];       //返回第n个位置上元素的引用
   v1 = v2;    //调用拷贝赋值运算符
   v1 == v2    //对应元素如果都相等，则返回true
   v1 = {a,b,c}; //用列表中的元素拷贝替换v1中的元素，完成后v1只有三个元素。
...
7. 不能使用下标形式添加元素：
   ...
   v1[0] = 4;  //错误，而在Python等语言中是支持这种操作的。
   ```

## 数组

1. 数组也是存放相同类型对象的容器，也需要通过下标来访问。和vector不同的是，数组的大小确定不变，运行时性能较好。
2. 数组是一种复合类型，数组大小属于数组类型的一部分，必须大于0，可以有空vector，但是不能有空数组。编译的时候数组大小应是已知的，也就是说数组大小必须是一个常量表达式：

   ```c++
   int arr1[10];      //数组有10个元素，每个都是int，默认初始化为0。
   int* arr2[10];     //数组有10个元素，每个都是int*，默认初始化为0。
   unsigned int cnt = 42;
...
7. 不能将数组的内容拷贝给其他数组进行初始化和赋值。一些编译器支持数组的赋值，这是编译器扩展，不建议使用。
   ...
   a2 = a;  //错误
   ```
8. 不允许vector来初始化数组，但是可以用数组来初始化vector。

   ```c++
   int int_arr[] = {0,1,2,3,4,5};
   vector<int> ivec(begin(int_arr),end(int_arr));  //将int_arr中的元素逐个用来初始化ivec中的元素。
   vector<int> ivec2(int_arr+1, int_arr+3);       //用int_arr[1]到int_arr[2]这2个元素初始化ivec2。不包括最后一个指针所指的元素。
   ```
9. 复杂的数组声明，定义数组的指针和引用，最好的方法是从名字开始由内向外阅读，数组的名字紧跟着维度，括号十分重要：
   ...
   ```
10. 数组的维度编译器知道，vector的维度可以由他的size函数获得。因此都可以使用范围for循环。
11. 使用数组时，编译器一般会将它转化为指向数组首元素的指针。因此关于数组的操作，很多时候就是指针的操作。用auto推断数组名时，会得到指针。但是会用decltype推断时，会得到数组类型。

    ```c++
    int ia[] = {1,2,3};
    auto ia2(ia); //ia2也是一个int*类型，指向ia[0]。等价于 auto ia2(&ia[0])，[]的优先级比&高。
    decltype(ia) ia3;
    //指针和数组最大区别不是是否可以用下标运算符，而是指针可以用另一个指针赋值。数组却不能用指针赋值。
    ia2[0] = 3;  //都可以使用下标运算符
    ia3[0] = 5;
    int *p = nullptr;
    ia2 = p;
...
12. 数组名可以看做是一级指针，但是数组名的地址并不是二级指针。因为数组名并不是一个变量，没有地址：
    ...
    int (*p3)[3] = &arr; //正确
    ```
13. 指向数组元素的指针可以使用迭代器支持的所有操作。C++11引入了两个函数begin和end，定义在iterator头文件中。接受数组名为参数，分别返回首元素指针和尾后指针。
14. 两个不同类型的指针无法相减，不同数组的指针相减没有意义。允许给空指针加上值为0的常量表达式，两个空指针可以相减，结果为0：

    ```c++
    int ia[] = { 1,2,3 };
    double ia2[] = { 1.1,2.2 };
    ia - ia2; //错误
    //同类型指针相减得到的并不是指针指相减的结果，而是该结果除以指针指向元素大小的结果，也就是两个指针之间的元素个数。
    int ia[] = { 1,2,3 };
    int *p = &ia[2];
    ptrdiff_t diff = p-ia;//结果是2。标准库使用ptrdiff_t类型存储两个指针的差，带符号类型。
    ```
15. 下表运算可以拆分为指针加减法和解引用的结合。

    ```c++
    int ia[] = { 1,2,3 };
    ia[2] == *(ia+2); //结果为true
    ```
16. 数组下标从0开始，类型为size_t，定义在cstddef头文件中，无符号类型。数组和vector都可以使用下标运算符[ ]，但是原因不同，数组的是C语言定义的，vector是标准库中重载了该运算符。
17. 标准库类型规定下标必须是无符号类型，而指针的下标可以是负值，只要指向的元素是原数组中有意义的元素即可。
...
1. 严格来说，C++中没有多维数组，通常说的多维数组其实是是元素为数组的数组。根据离数组名的距离远近，依次为第一，第二等维度。
...
4. 可以使用下标来访问多维数组中的元素。数组的每个维度对应于一个下标运算符。如果下标运算符的数量和数组的维度一样多，那么结果是给定类型的元素，如果下标运算符的数量较小，则是给定索引处的一个内层数组。
   ...
   ```c++
   int arr[2][3];
   int (&arr)[3] = arr[1]; //arr[1]是一个int[3]类型。
   ```
...
5. 用范围for处理多维数组：
   ...
   for (auto row : ia){  //此时row为 int*类型，分别为&ia[0][0]和&ia[1][0]。
   ...
   int ia[2][3] = { 0,1,2,3,4,5 };
   auto x = ia[0];  //x为int*类型
   auto &y = ia[0]; //y为引用类型，绑定到int[3]上
   ```

6. 使用多维数组名时，编译器会自动将其转换为数组首元素的指针，多维数组的首元素也是一个数组，因此结果是指向第一个内层数组的指针：

   ```c++
   int ia[3][4];
   int(*p)[4] = ia;  //正确，*p等于ia[0]。
   p = &ia[2];  //将p指向ia最后一个元素
   *p == ia[2]; //结果为true
   ```
...
2. 严格来说，string不属于容器，但是支持很多与容器类型类似的操作。

3. 类似于指针类型，迭代器提供了对元素的间接访问。迭代器可以移动，存在有效和无效之分。和指针不一样的是，获取迭代器不用取地址符，含有迭代器的类型都有成员可以返回迭代器。例如begin，end成员函数。begin迭代器指向第一个元素，end指向最后一个元素的下一个位置，也成为尾后迭代器。如果元素数量为0，则首迭代器和尾后迭代器相同，都是尾后。

4. 迭代器对象支持的运算：

   ```c++
   *iter;       //返回迭代器所指元素的引用，
   iter->mem;   //获取迭代器对象所指元素的mem成员，等价于(*iter).mem。必须要加括号，因为.运算符的优先级比*高。
   ++iter;      //令迭代器指向到下一个元素
   --iter;      //令迭代器指向到上一个元素
   ...
   iter1 == iter2;  //当且仅当 两个迭代器指向同一个元素，或它们是同一个容器的尾后迭代器时，才返回true。
   iter1 < iter2;   //如果iter1指向的元素更靠前，则返回true。
   ```

5. 由于尾后迭代器不指向一个具体的元素，因此不能对其进行递增和解引用。

6. C和Java程序员在for循环条件中喜欢使用<和使用下标来获取元素。而C++程序员更喜欢使用!=作为条件和用迭代器。C++标准库中的所有容器都是使用这种编程风格。标准库的所有容器的迭代器都定义了==和!=，但是大多数都没定义<。因此建议多使用!=和迭代器。

7. 拥有迭代器的容器会定义一个类类型，分别为iterator和const_iterator：

   ```c++
   vector<int>::iterator it1;  //it可以读写vector<int>中的元素
   vector<int>::const_iterator it2;  //it只能读取vector<int>中的元素。const_iterator和常量指针差不多
   ```
   
8. 如果容器是常量，则只能使用const_iterator，begin和end返回const_iterator；如果不是常量，可以使用iterator和const_iterator，begin和end返回iterator。

   ```c++
   const vector<int> v1; //const对象，空vector，不能向其中添加元素。
   vector<int> v2;
   auto a = v1.begin();  //a的类型为vector<int>::const_iterator
   auto b = v2.begin();  //b的类型为vector<int>::iterator
   auto c = v2.cbegin(); //也可以用cbegin,cend强制获取const_iterator，不论vector是否是常量。
   ```

9. -> 箭头运算符使C++特有的。将解引用和成员访问两个操作结合起来。

10. 任何一种可能改变容器大小的操作，都会使得迭代器失效。因此，使用了迭代器的循环体，都不应向容器内添加新元素。

12. 容器一般搭配迭代器使用。一般的智能指针只用处理*和->运算，而迭代器智能指针还需要处理++和--等普通指针可以进行的运算。

17. 双向链表形式的容器结点内部除了存储特定类型的对象，每个结点都还有两个指针，分别指向其前后的结点。迭代器对象内部有一个指针node，该指针指向容器内的某个结点。迭代器对象需要将++操作，转化为node指向下一个容器结点的行为。next和prev指针是指向结点的，而不是对应指针的。

18. 迭代器的使用者，不(也没有必要)知道prev，next的存在，因此对迭代器对象解引用时，应该获得的是data，而不是结点。

19. ![image-20210818105430616](C++.assets/image-20210818105430616.png)
...
7. <img src="C++.assets/image-20210815183212838.png" alt="image-20210815183212838" style="zoom: 80%;" />
...
3. <img src="C++.assets/image-20210815194702077.png" alt="image-20210815194702077" style="zoom:80%;" />
...
1. iostream库包含了两个类istream和ostream，分别表示输入流和输出流。流就是字符序列。流想要表达的含义是，随着时间的推移，字符是顺序生成或消耗的。

2. 标准库定义了4个IO对象，可以直接使用。系统常常将这些对象和所运行的窗口关联起来，因此当读写这些对象时就会读写窗口。
...
3. 一般来说，应该总是在输出语句的末尾刷新流，这样可以保证内容会及时地显示出来，这在程序崩溃时能更准确地定位出错位置。

4. \>\>可以看做是将左边的东西丢进右边。\<\<可以看做是将右边的东西丢进左边。而cin或cout只能出现在运算符左边。因此\>\>是标准输出，\<\<是标准输入。

5. cout可以看做是ostream子类的一个对象。
...
6. <\< 和 \>\>这两个称为流插入和流提取运算符，是左移，右移运算符的重载。std::cout是定义在\<iostream\>中的ostream类的对象。std::cin是该文件下的istream类的对象。这两个操作符的返回值都是左侧对象，因此可以连用。
...
8. cin的策略是，只要开始一次输入调用，就会允许用户输入，然后将输入的所有内容都存放到缓冲区中。然后每次从缓冲区出一个预定类型的数据赋值给变量，只要缓冲区还有内容，就会直接使用，而不是要求用户重新输入。

9. 当遇到 文件结束符EOF或无效的输入时，对象的状态就会变为无效，bool结果为false。

10. 通过键盘输入时，在Windows系统中，Ctrl+Z表示文件结束符，完事再按回车才会结束输入。Unix系统中Ctrl+D为文件结束符。
...
3. 编译器推断出来的auto类型有时和初始值的类型不一定完全一样：
   ...
   ```c++
   //将引用用作初始化，实际参与的是被引用的对象，因此推断出的类型也是被引用对象的类型，而非引用。
   int i = 0,&r = i;
   auto a = r;  //auto 为int
   ```

4. auto 会忽略掉顶层const，但是会保留底层const。
...
1. C++11引入了decltype类型说明符，作用是返回操作数的数据类型。编译器会分析表达式的类型，但是不会计算表达式的值。

   ```c++
   decltype (f()) a ; //获取f()的返回值的类型，定义一个同类型的变量a。并不会执行f函数。
   ```

2. 和auto不同，decltype保留顶层const和引用类型。
...
3. 如果对表达式使用decltype，而不是变量，则返回表达式结果对应的类型。
   ...
   ```c++
   decltype(r+0) x = 0; //表达式r+0结果的类型是int。所以x的类型是int
   //如果表达式的内容是解引用操作，则decltype的结果也是引用。
   int i = 42, *p  =&i, &r = i;
   decltype(*p) y = i;  //等价于 int &y = i;
   ```

4. decltype和auto的一个重要区别是，decltype和表达式的类型密切相关。变量名和变量名+括号不同，前者直接获得是变量的类型，后者会被当做一个表达式。变量是一种可以作为左值的特殊表达式。
...
1. C标准库的头文件在C++中仍然可以使用，不过都被包装成了C++的样子，例如给stdio.h套了一层壳就有了cstdio。同时将C标准库的名字都放到了std命名空间种。C++程序不推荐使用C的标准库。

   ```c++
   //VC下包含stdio.h或cstdio，就可以使用以下两种写法：
   printf("aaa");      //C语言的
   std::printf("abd"); //C++推荐的
   ```

2. 预处理变量不属于任何命名空间，它是由预处理器处理的，直接使用即可。

3. 头文件通常包含那些只能被定义一次的实体，例如类、const变量、constexpr变量。有了防卫式声明，可以确保这些实体只被定义一次。

4. 头文件也经常用到其他头文件的功能，例如xx.h用到了string类，则必须包含string头文件。xx.cpp也会用到string，也应包含string头文件。一般来说xx.cpp也会包含xx.h。因此xx.cpp就包含了两次string头文件。因此每个头文件都需要使用防卫式声明。

5. 预处理变量有两种状态，已定义和未定义。#define将一个名字定义为预处理变量，#ifdef 当且仅当变量已定义时返回true，#ifndef当且仅当变量未定义时返回true。两个#if都要和#endif配合使用，可以嵌套，但是不能交叉。

6. 整个程序中，每个头文件的防卫式声明必须唯一，通常使用类名，而且全大写，为了避免和程序中的其他实体发生冲突。

7. 标准库的头文件扩展名在不同的平台上可能不同。自定义的头文件也可以使用任意的扩展名，因为预处理器不检查扩展名。
...
9. 自定义的头文件需要有防卫式声明，为了使每个头文件只被引入一次，宏名前后都应该有两个下划线，这是为了和操作系统或标准库的头文件区分开。宏名应该和文件名一致，一般全大写。头文件名一般要和类名一致，建议都用小写，标准库的头文件都是小写。

10. 标准的头文件框架如下，一个头文件一般由一个或多个类组成：
...
11. 为某个类专门定义的普通函数（例如操作符重载函数），必须在头文件中声明，否则类的用户无法编译通过。如果只在头文件中声明的话，会报链接错误，即找不到函数，即使在其他地方定义也不行的。如果定义在.cpp文件中的话，要在头文件中对应的声明前加上extern。当然也可以直接在类的头文件中定义。前者是在链接时找到函数定义，后者是在编译时找到。
...
2. .hpp文件中不能包含全局对象和普通函数，因为他可能被多次#include，链接时就会造成符号重定义错误。需要去除全局对象，将普通函数封装为类的静态方法。
...
2. 类型别名和类型名等价，可以相互替换，但是遇到const和指针时，需要谨慎。
   ...
   const pstring cstr = 0; //cstr是顶层const，等价于char * const cstr = 0;不能将pstring直接替换进去。否则就是const char *cstr，变成底层const了。思路是先不替换pstring，将它肯做一个普通的类型，则cstr是pstring的顶层const常量对象。
...
1. namespace命名空间，类似于其他语言的模块和包，是为了避免重名。标准库所有的内容都被包含在std中，可以分段来写，最终会自动结合：
   ...
   using namespace std; //using指示。完全暴露，他的位置和头文件的包含位置没有关系，只要在使用前放置即可。
   cout;
...
3. 可以使用using声明，来简化步骤只是用命名空间中的名字，不过只有包含了相应的头文件才可以使用using来简化步骤：
...
4. 头文件中的代码一般来收不应该使用using声明，因为头文件会被拷贝到其他文件中去，可能会污染该文件中的名字。

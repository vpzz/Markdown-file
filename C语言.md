...
6. ![image-20210330194203364](C语言.assets/image-20210330194203364.png)

7. 条件预编译的宏名取法：如果文件名为xx.h，宏一般为XX\_H\_\_。因为内核中的头文件一般是前加下划线，用户程序使用后加下划线是为了和内核发生冲突。

8. #include 头文件的时候，<>包含的文件会直接去系统指定的目录中寻找，而" "包含的文件会现在当前路径下寻找，如果没找到，再去系统目录下寻找。

9. #pragma命令是用来设定编译器的状态，指定编译器完成一些特定的动作。

10. ![image-20210330194549153](C语言.assets/image-20210330194549153.png)

11. 广义的编译指的是源程序→可执行文件，狭义的编译指的是预处理后的文件→汇编文件。

12. 汇编过程是将汇编文件转化为机器代码的目标文件。

13. 链接是将使用到的外部库的函数实现找到，链接到可执行文件中。
...
2. <img src="C语言.assets/image-20210326181739691.png" alt="image-20210326181739691" style="zoom:50%;" />
3. 基本数据类型所占的字节数和机器的硬件有关，标准C中并没有规定基本数据类型的字节数，标准C中之规定了类型之间的字节数关系。通常来说，int，float和long类型都占一个字长大小。对于32位的机器，一个字长是4个字节。
4. C语言的变量赋值只能识别10,8,16进制，不识别二进制。254的2,8,16进制表示分别为B11111110 0376 0xFE。
...
3. 常量的分类：
   ...
   2. 实型，类似于1.2,3.2之类的数，还可以用科学计数法来表示 3.2e3，3.2E3，标准上没有规定类型。大多数编译器会将它看做是double类型。

      1. MSVC直接将实数常量当做double类型![image-20210402215132624](C语言.assets/image-20210402215132624.png)
...
6. ![image-20210402084646579](C语言.assets/image-20210402084646579.png)

7. 多个文件可以一起指定给gcc(gcc *.c)，也可以逐个编译，然后一起链接(gcc *.o)。实际上gcc在编译工程时，也是一个一个.c文件编译，然后再链接到一起的。在上面的例子中，编译main.c文件时，发现使用到的func函数在当前文件中有了声明（预处理引入的），就不会报编译错误。而链接时，由于该函数时static修饰的，因此只会在当前.o文件中查找函数的实现。结果并没有找到，所以会报未定义的错误。
...
2. 在当前.c中不声明会导致编译出错：
3. ![image-20210402095408056](C语言.assets/image-20210402095408056.png)
4. 如果使用extern声明的同时，进行初始化，例如：extern int a =100;  则编译时会报警告，链接时会报错误：
5. ![image-20210402095528835](C语言.assets/image-20210402095528835.png)
6. extern生命的变量不能进行初始化，类型也不能选错。不写类型也是可以的，例如extern i;
...
3. ![image-20210402100542848](C语言.assets/image-20210402100542848.png)

4. 运算符的优先级和结合性：

5. ![image-20210402100700029](C语言.assets/image-20210402100700029.png)
...
1. I/O操作分为标准I/O和文件I/O。

2. 格式化输出和输入函数都是变参函数：

   ```c
   int printf(const char *format, ...);      //printf自己也不知道有多少个参数，因此参数和格式字符不对应也可以。
   %[修饰符]格式字符
   ```

3. 有符号变量使用%u输出时，变量的值应该是正数。无符号变量使用%d输出时，应该在有符号数的范围内。

4. %f默认保留6位小数，不够的补0，多余的截去。实际是float精度，%lf是double精度。要读入double类型，必须要用%lf来读入，以免精度丢失。而输出，由于printf中并无对%lf的严格定义，故使用%lf不一定会出现正确结果。由于c语言中的默认参数提升规则，%f输出的不论是float还是double都会被提升到双精度进行输出，并不会有精度丢失。
...
6. ![image-20210402143041794](C语言.assets/image-20210402143041794.png)

7. 修饰字符如下：.n的保留小数是四舍五入。
...
8. ![image-20210402145632597](C语言.assets/image-20210402145632597.png)

9. scanf的format中的格式字符和printf的相同。不同的是，后面的变参数为地址表。scanf的提示一般使用用printf输出。如果需要接受多项输入，应该提示用户每项之间的分隔符。系统会将用户的输入去和scanf的format参数比对，因此该参数中不应有\n，只有格式字符和分隔符就好了。例如：
...
10. scanf之所以要传地址，是因为要在函数内修改i和j的值，如果传的是i和j，由于值传递，在函数给i和j赋值后，并不会影响外边的i和j。这是典型的将结果当做输入参数的例子。

11. 如果format中的多个参数之间不加分隔符，那么用户输入时，可以用回车，tab键，空格三个中的任意一个。
...
12. scanf中使用%s来接受一个字符串是非常危险的，因为程序为接受该字符串开辟的空间可能不够，进而发生溢出，覆盖掉有用的区域。

13. scanf使用%s来接受字符串时，用户的输入中间不能有空格，否则会只取第一段。要想读入有空格的字符串，可以用sscanf。

14. printf如果成功，返回输出的字符个数，不包括字符串的尾零。scanf如果成功，返回成功读取匹配到的项数。

15. 一般情况下，应该校验scanf的成功与否，然后才可以使用读取到的变量。

16. 字符输入输出函数（putc getc），字符串输入输出函数（gets puts）见系统编程中的描述。尽量不要使用gets函数，因为该函数不检查缓冲区溢出，用fgets，它会检查输入的长度，如果超过了size参数，就会提示错误。这样只能保证不出错，并不能保证接受用户的所有输入。

17. 在GNU C中，存在getline函数，可以获取用户输入的一行数据，因为使用了动态内存技术，因此能保证接受用户的所有输入。
...
12. 在终端按下Ctrl+C，可以给当前进程发送一个SIGNAL INTERRUPT的信号，杀死进程。

13. break（跳出本层循环）和continue（跳出本次循环）只在训话或switch-case语句中才有用。
...
8. 从一维数组来看，arr[i] 是对数组元素的直接访问，*(arr+i)是对数组元素的间接访问。arr+i是int *类型的，表示是一个指向int类型值的指针。
   ...
   *arr == arr[0];  //*arr会先将arr转化为一个指针，然后再取值。
   ...
   ![image-20210405132317303](C语言.assets/image-20210405132317303.png)
...
15. ==![image-20210405001419100](C语言.assets/image-20210405001419100.png)==
...
7. ![image-20210405155738233](C语言.assets/image-20210405155738233.png)

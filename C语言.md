...
14. 可以用make来编译一个文件，例如make hello，会执行 gcc hello.c -o hello。

15. -Wall 选项可以打开gcc的所有警告和报错开关，最严格的模式。一般来说，应该尽可能修改程序到所有的警告都消失，或者说是可控的警告。

16. 使用任何的外部的函数都要包含头文件的定义。
...
18. 有时候出现段错误的原因就隐藏在没有解决的警告中。例如下面的程序如果去掉对string.h的包含，编译时没有任何警告，运行时会报段错误。如果打开-Wall选项后，编译时回报警告，提示strerror的返回值和用于接受的变量类型不匹配，包含上该头文件就没事了。
...
19. 如果编译的工程中有多个.c文件中都存在main函数，gcc也会报错。

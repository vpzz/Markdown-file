...
36. 复制Windows中文件名的推荐方法：在文件上按住shift然后右键，选择复制文件地址，结果会自动带双引号，使用\分割，例如`"C:\Users\zj\Desktop\1.txt"`。不要使用右键→安全来复制文件名，因为Windows（以及整个Unicode标准）为了正确显示混合书写方向的文本（比如英文+阿拉伯文/希伯来文）而引入的一类不可见的Unicode双向控制字符，U+202A。
...
19. 可以将一篇文章里的所有origin图片都汇总到一个project中，然后统一设置格式，可以一次性全部导出。

20. 图片的grid一般水平5份，竖向4份。或者4:4。
...
63. APA规范要求，标题中的单词除了冠词，并列连词，介词外首字母都要大写，首尾单词总是首字母大写。不要加粗例如：The Symbolic Importance of Words in O'Keefe's Work

65. 外国人写作也缩进，APA规定是0.5 inch

66. 英文名字一般是先名后性，例如 Jian Zhang或者可以先姓后名，中间用逗号分隔，例如Zhang,Jian。

67. 科研中原始图片的类型：实验图（picture，用相机拍摄或者电子显微镜等设备导出），数据图（graph，origin导出的线，散点图），示意图（drawing，Illustrate导出的）
...
7.  只有存在公共节点，才能有公共面，才能出现对齐的网格。
8.  Add：结果是一个新的图元，单一的整体，没有接缝。
9.  <img src="杂记.assets/1589294629977.png" alt="1589294629977" />
10.  Intersect：交运算，由输入图元的重复区域组成。
11.  <img src="杂记.assets/1589294638467.png" alt="1589294638467" />
12.  Substract：减运算，同维数的图元之间进行。
13.  <img src="杂记.assets/1589294654363.png" alt="1589294654363" />
14.  Divide：分割运算，高维图元被低维图元分割。
15.  <img src="杂记.assets/1589294660500.png" alt="1589294660500" />
16.  Overlap：重叠运算，重叠区域必须与原图元有相同的维数。
17.  <img src="杂记.assets/1589294668200.png" alt="1589294668200" />
18.  Glue：粘接运算，图元尽在公共边界处相交，粘接后，这些图元依然独立，只不过在边界处相互连接。
19.  <img src="杂记.assets/1589294682306.png" alt="1589294682306" />
20.  <img src="杂记.assets/1589294688715.png" alt="1589294688715" />
21.  <img src="杂记.assets/1589294693596.png" alt="1589294693596" />
22.  ANSYS jobname显示在软件的最上边，也是.db文件的名称。操作界面下的是title。
23.  <img src="杂记.assets/1589294701678.png" alt="1589294701678" />
24.  ANSYS的select entity可以选择出来一部分单元或图元，然后可以将他们创建为component，以备之后的使用。
...
3.  CPU的主要参数有插槽类型，架构，核心数，频率，缓存，热功耗设计。
...
15.  CPU和主板的搭配要满足两条：①CPU针脚和主板插槽一致，②主板芯片组与CPU兼容。
...
45.  小主板只是扩展接口少，性能没差距的
48.  AMD和NVIDIA生产核心，同时自己也会除公版显卡。做工中规中矩，外观朴实。
...
3. 最早的C语言的编译器：最早的C编译器A是用汇编语言写的，然后用再用C语言写编译器，用编译器A来编译，得到编译器B。再往后就可以脱离汇编语言了，直接用C语言修改更新编译器。
...
28. 当中间主应力在第一和第三主应力之间变化时，lode参数在-1到1之间变化。通过lode参数引入中间主应力影响系数$\beta$，可以统一Tresca和Mises准则。变化范围是1到1.155。

29. $$
    \beta=\frac{2}{\sqrt{3+{\mu_\sigma}^2}}
    $$

30. 先有从实验中总结出来的Tresca准则，然后Mises在运用中觉得六边形不如圆形好用，便用圆形替代，lode通过实验证实，Mises准则和实验更贴合。
...
38. 1950年前后，实验验证了圣维南原理的正确性。这样才可以用杆表面测得的应变脉冲代表杆内部传播的应变脉冲。
...
40. ppm 百万分比浓度，是用溶质质量占全部溶液质量的百万分比来表示的浓度。    mg/L 在溶液密度为1时，等于ppm。

41. 四舍六入五成双是一种比较精确比较科学的计数保留法，是一种数字修约规则。
...
42. 箱形图最大的优点就是不受异常值的影响（异常值也称为离群值），可以以一种相对稳定的方式描述数据的离散分布情况。采用5个数来描述序列数据，最大值，Q1，中位数，Q3，最小值。

43. <img src="杂记.assets/d3b47785-d6c0-4fe7-a607-b9755d2d6d06.png" alt="img" />

44. 首先将数据进行切割，分为4份。Q1,Q2（中位数 median）,Q3为分割线。     平均数（mean）一般显示为一个点。

45. 四分位距IQR=Q3-Q1。上限是非异常范围内的最大值。=Q3+1.5IQR。下限是非异常范围内的最小值。=Q1-1.5IQR。内限就是将异常值去除掉之后的上下限。

46. 例如：1,6,2,7,4,2,3,3,8,25,30 这样一个序列，统计如下：
